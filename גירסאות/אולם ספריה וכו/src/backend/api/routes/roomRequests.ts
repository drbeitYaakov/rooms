...
router.post('/groups', authMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const effectiveEndDate = effectiveStartDate ? getSchoolYearEndForDate(effectiveStartDate) : normalizedEndDate;
  ...
  for (const occurrence of occurrences) {
    await ensureHomeroomAssignmentsForDate(occurrence.date, req.user!.id);
  }

  const [rooms, homerooms, assignments] = await Promise.all([
    db('rooms')
      .select(
        'id',
        'room_number',
  ...
  try {
    await db.transaction(async (trx) => {
      ...
      const now = new Date().toISOString();
      const auditoriumDefaultOccurrences: Array<{ date: string; start_time: string; end_time: string; room_id: string | number; room_number: string; room_location: string; request_id: string | number; explanation: string[]; alerts: string[] }> = [];
      ...
      if (isHighSchoolPe && auditoriumDefaultOccurrences.length > 0) {
        ...
        await applyAuditoriumDefaultSettingsToAssignments(trx as any, [auditoriumRoomId], effectiveStartDate, req.user!.id);
