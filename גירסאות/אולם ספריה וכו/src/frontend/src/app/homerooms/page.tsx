...
type TabKey = "homerooms" | "fixed-times";
...
export default function HomeroomsPage() {
  ...
  useEffect(() => {
    void Promise.all([loadHomerooms(), loadGrades(), loadDefaults()]).finally(() => setLoading(false));
  }, []);
  ...
  const loadFilteredRooms = async (gradeId: string) => {
    ...
    const response = await authenticatedFetch(`http://localhost:3001/api/homerooms/available-rooms?grade_id=${gradeId}&school_year=${encodeURIComponent("תשפ\"ד")}`);
    const data = await response.json();
    if (!data.success) throw new Error(data.error || "failed");
