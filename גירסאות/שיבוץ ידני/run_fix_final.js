// סקריפט פשוט שמשתמש ישירות ב-knexfile
// מפעיל את התיקון במסד הנתונים
...
try {
  ...
  async function runFix() {
    try {
      ...
      // בדיקת סטטיסטיקות
      const totalResult = await db('assignments').where('status', 'active').count('* as total');
      const uniqueResult = await db('assignments')
        .where('status', 'active')
        .select(db.raw('COUNT(DISTINCT room_id || DATE(date) || start_time || end_time) as unique_slots'))
        .first();
  ...
  console.log('\n🔧 פתרונות:');
  console.log('1. בדוק שהקובץ knexfile.js קיים');
  console.log('2. בדוק את הגדרות מסד הנתונים ב-.env');
