...
const ACTIVITY_OPTIONS = [
  ...
  { value: HIGH_SCHOOL_PE_ACTIVITY_TYPE, label: "התעמלות תיכון" },
];
          ...
          <div className="w-full max-w-3xl rounded-[28px] bg-white p-6 shadow-[0_24px_80px_-32px_rgba(15,23,42,0.45)] ring-1 ring-slate-200">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  כדי לפנות מקום לחלק מהמופעים, המערכת העבירה שיבוצים קיימים לחדרים חלופיים. חשוב לאשר שקראת את העדכון.
                </p>
              </div>
            ...
            <div className="mt-5 space-y-4">
              {relocatedOccurrences.map((occurrence) => (
                <div key={`${occurrence.assignment_id}-${occurrence.date}-${occurrence.start_time}`} className="rounded-2xl border border-sky-200 bg-sky-50 px-4 py-4">
                  ...
                  <div className="mt-3 space-y-2">
                    {occurrence.relocated_assignments?.map((item, index) => (
                      <div key={`${item.assignmentId}-${index}`} className="rounded-xl bg-white/80 px-3 py-2 text-sm text-sky-900 ring-1 ring-sky-100">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">
                המערכת תחפש קודם את החדר שמתאים למספר הגדול ביותר של המופעים, ואז תשלים את שאר המועדים בחדרים חלופיים לפי אותם כללי שיבוץ של בקשה חד-פעמית.
              </p>
            </div>
                    ...
                    <div className="mt-4">
                      ...
                      <ul className="mt-2 space-y-2 text-sm text-slate-700">
                        {assignment.explanation.map((reason, index) => (
                          <li key={`${assignment.assignment_id}-reason-${index}`} className="rounded-xl bg-white px-3 py-2 ring-1 ring-slate-200">
                            {reason}
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="mt-4">
                      ...
                      <ul className="mt-2 space-y-2 text-sm text-amber-800">
                        {assignment.alerts.map((alertText, index) => (
                          <li key={`${assignment.assignment_id}-alert-${index}`} className="rounded-xl bg-amber-50 px-3 py-2 ring-1 ring-amber-100">
                            {alertText}
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="mt-4">
                      ...
                      <ul className="mt-2 space-y-2 text-sm text-sky-800">
                        {assignment.relocated_assignments.map((item, index) => (
                          <li key={`${assignment.assignment_id}-relocation-${index}`} className="rounded-xl bg-sky-50 px-3 py-2 ring-1 ring-sky-100">
                            {item.activityType}: {item.previousRoomNumber} הועבר אל {item.newRoomNumber} ({item.location})
                          </li>
                        ))}
                      </ul>
