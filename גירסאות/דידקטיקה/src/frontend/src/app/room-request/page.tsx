  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setSubmitError(data.error || "לא ניתן היה להשלים את השיבוץ.");
        setResult({
          message: data.error || "לא ניתן היה להשלים את השיבוץ.",
          room: {
            id: "",
            room_number: "",
            capacity: 0,
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  כדי לפנות מקום לבקשת הדידקטיקה, המערכת העבירה שיבוץ קיים לחדר חלופי. חשוב לאשר שקראת את העדכון.
                </p>
              </div>
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-slate-600">
                בחרי את סוג השיבוץ הרצוי. אפשר לעבור בין בקשה חד פעמית לבין שיבוץ מתקדם בלי לצאת מהמסך.
              </p>
            </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
              ...
              <p className="mt-2 text-sm text-slate-600">
                השיבוץ מתבצע לפי סוג הקבוצה, הצורך במקרן, סדרי העדיפויות, כללים מיוחדים, התרעות למשתמש וחלופות אפשריות.
              </p>
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <div>
                    ...
                    >
                      ...
                      <option value="didactics">דידקטיקה</option>
                      <option value="exam_makeup">השלמת מבחנים</option>
                      <option value="one_on_one">ישיבה אחד על אחד</option>
                      <option value="discussion">שיח</option>
                      <option value="topics">סוגיות</option>
                      <option value="study_group">הקבצה</option>
                      <option value="meeting">מפגש</option>
                  ...
                  <label className="flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-600">
                        כאשר מסמנים, הבדיקה תעדיף רק חדרים עם מקרן ותסביר זאת גם בתוצאת השיבוץ.
                      </p>
                    </div>
                    <input
                      type="checkbox"
            ...
            <aside className="space-y-6">
              <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                ...
                {suggestedRoom ? (
                  <p className="mt-3 text-sm leading-6 text-slate-700">
                    לפי מספר התלמידים{formData.needsProjector ? " והצורך במקרן" : ""}, חדר מתאים יכול להיות{" "}
                    <strong>{suggestedRoom.room_number}</strong> עם קיבולת של <strong>{suggestedRoom.capacity}</strong>.
                  </p>
                ) : (
                  <p className="mt-3 text-sm leading-6 text-slate-600">
                    לאחר מילוי פרטי הקבוצה נציג כאן התאמה ראשונית, ולחיצה על השיבוץ תריץ את הלוגיקה המלאה עם כל הכללים וההתראות.
                  </p>
                )}
              </section>
              ...
              <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                ...
                <div className="mt-3 space-y-2 text-sm leading-6 text-slate-700">
                  <p>המערכת בודקת קודם כל כללים מיוחדים לפי סוג קבוצה, ורק אחר כך את סדרי העדיפויות הכלליים.</p>
                  <p>התוצאה כוללת חדר נבחר, קומה ואגף, נימוק לבחירה, התרעות למשתמש וחלופות אפשריות.</p>
                </div>
              </section>
                ...
                <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                  ...
                  {result.room.room_number && (
                    <div className="mt-4 rounded-2xl bg-emerald-50 p-4 ring-1 ring-emerald-100">
                      <div className="text-sm font-semibold text-emerald-900">חדר משובץ: {result.room.room_number}</div>
                      {result.location && <div className="mt-1 text-sm text-emerald-800">מיקום: {result.location}</div>}
                    </div>
                  )}
