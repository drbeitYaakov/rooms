...
function extractRecurringGroupName(specialRequirements: unknown): string | undefined {
  ...
  try {
    ...
    return typeof parsed?.recurring_group_name === 'string' && parsed.recurring_group_name.trim() !== ''
      ? parsed.recurring_group_name.trim()
      : undefined;
  } catch {
    return undefined;
  }
...
function isNumericIdentifier(value: unknown): value is string {
...
function getAssignmentPriorityForDisplay(assignment: any, targetDate: string): number {
  ...
  if (specificDate === targetDate && !isHomeroomAssignment) {
    return 4;
  }

  if (assignment.type === 'one_time' && !isHomeroomAssignment) {
    return 3;
  }
...
function getManualAuditoriumTitle(assignment: any, roomType: unknown): string | undefined {
  if (String(roomType || '').toUpperCase() !== 'AUDITORIUM' || assignment.is_manual !== true) {
    return undefined;
  }

  if (assignment.activity_type === 'didactics') {
    return 'הרצאת שכבה';
  }

  if (assignment.activity_type === 'event') {
    return 'אירוע באישור הנהלה';
  }

  return undefined;
}

function enrichAssignmentForDisplay(
  assignment: any,
  studyGroupMap: Map<string, any>,
  roomRequestNameMap: Map<string, string>,
  roomTypeById: Map<string, string>
) {
  const studyGroup = assignment.assignable_type === 'study_group'
    ? studyGroupMap.get(String(assignment.assignable_id))
    : undefined;
  const recurringGroupName = roomRequestNameMap.get(String(assignment.assignable_id));
  const auditoriumMetadata = extractAuditoriumMetadata(assignment.time_slots);
  const manualAuditoriumTitle = getManualAuditoriumTitle(assignment, roomTypeById.get(String(assignment.room_id)));
  const displayTitle = auditoriumMetadata.title || manualAuditoriumTitle;

  return {
    ...assignment,
    study_group_name: assignment.study_group_name || displayTitle || studyGroup?.name || recurringGroupName,
    assignment_title: displayTitle,
    assignment_note: auditoriumMetadata.note,
    grade: assignment.grade || studyGroup?.grade_level,
    student_count: assignment.student_count ?? studyGroup?.student_count,
...
router.get('/grid', authMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const roomRequestIds = Array.from(
    new Set(
      assignments
        .filter((assignment) =>
          assignment.assignable_type !== 'study_group' &&
          assignment.assignable_type !== 'homeroom' &&
          isNumericIdentifier(String(assignment.assignable_id ?? ''))
        )
        .map((assignment) => String(assignment.assignable_id))
    )
  );

  const roomRequests = roomRequestIds.length > 0
    ? await db('room_requests')
        .select('id', 'special_requirements')
        .whereIn('id', roomRequestIds)
    : [];

  const roomRequestNameMap = new Map(
    roomRequests
      .map((request: any) => [String(request.id), extractRecurringGroupName(request.special_requirements)] as const)
  ...
  const calendarGrid = rooms.map(room => {
    ...
    dates.forEach((date: string) => {
      ...
      timeSlots.forEach((timeSlot: string) => {
        ...
        const prioritizedAssignments = [...matchingAssignments].sort((left, right) => {
          const priorityDiff =
        ...
        const assignmentsForDisplay = prioritizedAssignments.map((assignment) =>
          enrichAssignmentForDisplay(assignment, studyGroupMap, roomRequestNameMap, roomTypeById)
        );

        const specificAssignment = assignmentsForDisplay[0];
        ...
        roomSchedule.schedule[date][timeSlot] = {
          ...
          assignment: occupyingAssignment ? {
            ...
            assignment_title: occupyingAssignment.assignment_title,
            assignment_note: occupyingAssignment.assignment_note,
          ...
          assignments: occupyingAssignment
            ? (specificAssignment
                ? assignmentsForDisplay.map((assignment) => ({
                    id: assignment.id,
                    study_group_name: assignment.study_group_name,
                    ...
                    assignable_type: assignment.assignable_type,
                    is_default_homeroom: assignment.is_default_homeroom,
