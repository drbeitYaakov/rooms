...
export class SchedulingEngine {
  private getRoomNumber(room: any): string {
    return String(room?.room_number || room?.roomNumber || '');
  }

  private getRoomPriorityType(room: Room): string {
    const roomNumber = this.getRoomNumber(room);

    return classifyRoomPriorityType({
      room_type: room.room_type,
      notes: (room as any).special_notes ?? null,
      room_number: roomNumber,
      grade_level: (room as any).grade_level ?? null
    });
  }

  private getRoomPreferenceRank(
    settings: RoomPrioritySettingsPayload | null,
    room: Room,
    ...
    const roomOrder = resolveRoomPriorityOrder(settings, {
      room_type: this.getRoomPriorityType(room) as any,
      date: assignmentRequest.date,
      start_time: assignmentRequest.start_time,
      end_time: assignmentRequest.end_time
    });

    const rank = roomOrder.findIndex((roomId) => roomId === String(room.id));
  ...
  async assignRoom(
    ...
    const rooms = availableRooms.map(roomEntity => ({
      ...
      room_number: roomEntity.room_number || roomEntity.roomNumber,
      room_type: this.mapRoomType(roomEntity.roomType),
      floor: roomEntity.floor,
      wing: roomEntity.wing,
      capacity: roomEntity.capacity,
      has_projector: roomEntity.has_projector ?? roomEntity.hasProjector,
      is_small: roomEntity.is_small ?? roomEntity.isSmall,
      comfort_priority: this.mapPriority(roomEntity.priority),
      special_notes: roomEntity.special_notes ?? roomEntity.notes,
      is_active: roomEntity.is_active ?? roomEntity.isActive,
      created_at: roomEntity.created_at ?? roomEntity.createdAt
    }));
    ...
    const roomPrioritySettings = await loadRoomPrioritySettings(db);
  ...
  private async findBestRoomForAssignment(
    ...
    // Preserve the existing business scoring, and only use preferences
    // to break ties between rooms from the same category.
    return candidates.sort((a, b) => {
      const leftType = this.getRoomPriorityType(a.room);
      const rightType = this.getRoomPriorityType(b.room);

      if (leftType === rightType && Math.abs(a.score - b.score) < 10) {
        const leftRank = this.getRoomPreferenceRank(roomPrioritySettings, a.room, assignmentRequest);
        const rightRank = this.getRoomPreferenceRank(roomPrioritySettings, b.room, assignmentRequest);
      ...
      return this.getRoomNumber(a.room).localeCompare(this.getRoomNumber(b.room));
    });
  }
