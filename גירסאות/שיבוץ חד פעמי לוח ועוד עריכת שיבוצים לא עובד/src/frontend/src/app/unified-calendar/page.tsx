...
const isHomeroomAssignment = (assignment: DayAssignment) =>
  assignment.id === "default-homeroom" || assignment.assignable_type === "homeroom";
...
export default function UnifiedCalendarPage() {
  ...
  const fetchCalendarData = async () => {
    try {
      ...
      data.data.rooms.forEach((room: Room & { schedule?: Record<string, Record<string, { is_occupied: boolean; assignment?: DayAssignment }>> }) => {
  ...
  const getRoomAssignmentsForDay = (roomId: string, date: string) => {
    ...
    calendarData
      .filter((cell) => cell.room_id === roomId && cell.date === date && cell.is_occupied && cell.assignment)
      .forEach((cell) => {
        if (!cell.assignment) {
          return;
        }
        ...
        groupedAssignments[key] = cell.assignment;
      });
  ...
  const handleDeleteAssignment = async (assignment: DayAssignment, date: string, roomId: string) => {
    ...
    try {
      ...
      const response = isHomeroomAssignment(assignment)
  ...
  const handleSaveAssignmentEdit = async () => {
    ...
    try {
      ...
      const response = isHomeroomAssignment(editingAssignment.assignment)
                          ...
                          {visibleDays.map((day) => {
                            ...
                            return (
                              <div
                                ...
                                dir="ltr"
                                className="relative min-h-[108px] overflow-visible border-l border-slate-200 bg-[linear-gradient(180deg,_rgba(248,250,252,0.95),_rgba(255,255,255,1))] px-3 py-3"
                              >
                                ...
                                {assignments.map((assignment) => {
                                  ...
                                  const homeroom = isHomeroomAssignment(assignment);
                                  ...
                                  return (
                                    <div
                                      ...
                                      style={{
                                        left: `${position.left}%`,
                                        width: `${Math.max(position.width, homeroom ? 16 : 12)}%`,
                                        top: homeroom ? "3.75rem" : "0.9rem",
                                        minWidth: homeroom ? "180px" : "160px",
                                        height: homeroom ? "2.3rem" : "3.1rem",
                                        zIndex: homeroom ? 8 : 14,
                                      }}
                                    ...
                                    >
                                      ...
                                      <div className={`flex h-full flex-col justify-between px-3 ${homeroom ? "py-1.5" : "py-2"}`} dir="rtl">
                                        <div className="truncate pl-7 text-sm font-bold">{label}</div>
