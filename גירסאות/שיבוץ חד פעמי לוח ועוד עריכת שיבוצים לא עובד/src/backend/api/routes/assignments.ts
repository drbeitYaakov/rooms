...
router.post('/homeroom-default', authMiddleware, requireCoordinator, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const conflictingAssignments = await db('assignments')
    ...
    .andWhereNot(function() {
      this.where('assignable_type', 'homeroom')
        .andWhere('assignable_id', homeroom.id);
    });
  ...
  const existingOverride = await db('assignments')
    .where({
      room_id,
    ...
    .whereRaw('DATE(specific_date) = DATE(?)', [targetDate])
    .first();
  ...
  try {
    updatedAssignment = await db.transaction(async (trx) => {
      ...
      if (!isRecurringAssignment(assignment)) {
        const nextTimeSlots = JSON.stringify([{ start: start_time, end: end_time }]);
