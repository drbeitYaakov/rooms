              ...
              <div className="mb-6 rounded-lg bg-white shadow">
                <div className="px-4 py-5 sm:p-6">
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <button onClick={handleScheduleGroups} disabled={selectedGroups.length === 0} className="rounded-md bg-green-600 px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-green-700 disabled:bg-gray-400">
...
function WeeklyScheduleEditor({
  ...
  const toggleDay = (dayOfWeek: number, enabled: boolean) => {
    onChange(
      enabled
        ? [...weeklySchedule, { day_of_week: dayOfWeek, start_time: "08:00", end_time: "08:45" }]
        : weeklySchedule.filter((entry) => entry.day_of_week !== dayOfWeek)
    );
  };

  const updateDayField = (dayOfWeek: number, field: "start_time" | "end_time", value: string) => {
    onChange(
    ...
    <div className={compact ? "space-y-2 text-xs" : "space-y-3"}>
      <div>
        ...
        <div className="mt-1 space-y-2 rounded-md border border-gray-300 p-3">
          {DAY_OPTIONS.map((day) => {
            const scheduleEntry = weeklySchedule.find((entry) => entry.day_of_week === day.value);

            return (
              <div key={day.value} className={compact ? "space-y-1" : "grid grid-cols-[120px_1fr] items-center gap-3"}>
                <label className="flex items-center gap-2 text-sm text-gray-700">
                  <input
                    type="checkbox"
                    checked={Boolean(scheduleEntry)}
                ...
                {scheduleEntry && (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(event) => updateDayField(day.value, "start_time", event.target.value)}
                    ...
                    <input
                      ...
                      onChange={(event) => updateDayField(day.value, "end_time", event.target.value)}
                      className="rounded-md border border-gray-300 px-2 py-1"
                    />
                  </div>
                )}
