      ...
      <div className={`rounded-md border p-4 ${result.success ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}>
        ...
        {result.scheduling_window && (
          <p className={`mt-2 text-sm ${result.success ? "text-green-800" : "text-red-800"}`}>
            טווח שיבוץ: {new Date(result.scheduling_window.start_date).toLocaleDateString("he-IL")} עד {new Date(result.scheduling_window.end_date).toLocaleDateString("he-IL")}
          </p>
        )}
      </div>
            ...
            {groupedAssignments.map((group) => (
              <div key={group.group_id} className="rounded border border-blue-100 bg-white p-3">
                ...
                <div className="mt-1 text-sm text-gray-600">
                  תאריכים: {group.dates.map((date) => new Date(date).toLocaleDateString("he-IL")).join(", ")}
                </div>
              </div>
            ))}
          </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {detailedAssignments.map((assignment) => (
                  <tr key={assignment.id}>
                    ...
                    <td className="px-3 py-2 text-gray-700">{new Date(assignment.date).toLocaleDateString("he-IL")}</td>
