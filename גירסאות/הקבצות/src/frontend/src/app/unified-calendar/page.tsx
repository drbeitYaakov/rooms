...
export default function UnifiedCalendarPage() {
  ...
  const isDefaultHomeroomAssignment = (assignment: DayAssignment) => {
    return assignment.id === 'default-homeroom';
  };

  const getDisplayAssignmentsForDay = (roomId: string, date: string) => {
    return getRoomAssignmentsForDay(roomId, date);
  };
  ...
  const getAssignmentPosition = (assignment: DayAssignment) => {
    ...
    return {
      left: ((totalMinutes - endMinutes) / totalMinutes) * 100,
      width: ((endMinutes - startMinutes) / totalMinutes) * 100
    };
  };

  const getAssignmentLabel = (assignment: DayAssignment) => {
    return assignment.study_group_name || getActivityTypeText(assignment.activity_type);
  };

  const getAssignmentBlockClassName = (assignment: DayAssignment) => {
    const baseClassName = getActivityTypeColor(assignment.activity_type);
    return isDefaultHomeroomAssignment(assignment)
      ? `${baseClassName} ring-1 ring-orange-300/70 shadow-md`
      : baseClassName;
  };
                  ...
                  <div key={room.room_id} className="flex min-h-[50px] relative">
                    ...
                    {weekDays.map((day, index) => (
                      <div key={`${room.room_id}-${day.date}`} className="relative border-l border-r border-t border-gray-200 p-4 min-h-[50px] min-w-[1400px]">
                        ...
                        {(() => {
                          const assignments = getDisplayAssignmentsForDay(room.room_id, day.date);
                          return assignments.map((assignment) => {
                            if (!assignment.start_time || !assignment.end_time) {
                              console.log('Skipping assignment due to missing time:', assignment.id);
                              return null;
                            ...
                            const isDefaultHomeroom = isDefaultHomeroomAssignment(assignment);
                            
                            return (
                              <div
                                key={assignment.id}
                                className={`absolute rounded-md text-sm border shadow-sm overflow-visible ${getAssignmentBlockClassName(assignment)}`}
                                style={{
                                  left: `${position.left}%`,
                                  width: `${position.width}%`,
                                  top: isDefaultHomeroom ? '2.9rem' : '0.75rem',
                                  height: isDefaultHomeroom ? '3rem' : '4rem',
                                  zIndex: isDefaultHomeroom ? 6 : 16,
                                  minWidth: isDefaultHomeroom ? '220px' : '190px'
                                }}
                                title={`${label} (${assignment.start_time}-${assignment.end_time})`}
                              >
                                <div className={`flex h-full flex-col justify-between px-3 ${isDefaultHomeroom ? 'py-1.5' : 'py-2'}`}>
                                  <div className="truncate font-bold text-sm leading-4">
                                    {label}
                                  </div>
                                  {!isDefaultHomeroom && (
                                  ...
                                  {assignment.grade && !isDefaultHomeroom && (
                                    <div className="truncate text-[11px] opacity-75">
                                      שכבה {assignment.grade}
                                    </div>
                                  )}
                                  {isDefaultHomeroom && (
                                ...
                                {!isDefaultHomeroom && (
                                  <div className="absolute -top-3 right-2 rounded-full bg-white px-2 py-0.5 text-[11px] font-semibold text-gray-700 border border-gray-300 shadow-sm">
                                    {assignment.start_time}
                                  </div>
                                )}
                                {!isDefaultHomeroom && (
                                  <div className="absolute -top-3 left-2 rounded-full bg-white px-2 py-0.5 text-[11px] font-semibold text-gray-700 border border-gray-300 shadow-sm">
                                    {assignment.end_time}
                                  </div>
                                )}
                              </div>
