...
export class SchedulingEngine {
  ...
  private async isRoomAvailableForAssignment(
    ...
    // Check default homeroom occupancy
    const isHomeroom = room.room_type.startsWith('CLASSROOM_');
    if (isHomeroom) {
      // Get day of week for the date (0=Sunday, 6=Saturday)
      const dayOfWeek = new Date(date + 'T00:00:00').getDay();
      
      // Only check school hours on weekdays (Sunday-Friday), not on Saturday
      if (dayOfWeek >= 0 && dayOfWeek <= 5) { // 0=Sunday, 5=Friday
        // Homerooms are occupied by default from 8:00 to 14:40 on weekdays
        ...
        // Homeroom school hours: 8:00 (480 min) to 14:40 (880 min)
        const schoolStartMinutes = 8 * 60; // 480
        const schoolEndMinutes = 14 * 60 + 40; // 880
        
        // Check if requested time conflicts with school hours using proper overlap logic
        const hasConflict = (startTimeInMinutes < schoolEndMinutes) && (endTimeInMinutes > schoolStartMinutes);
        
        if (hasConflict) {
          console.log(`Homeroom conflict: room ${roomId} requested ${startTime}-${endTime} conflicts with school hours 08:00-14:40 on day ${dayOfWeek}`);
          return false;
        }
      }
    }
