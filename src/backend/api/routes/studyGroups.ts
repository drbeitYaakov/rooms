...
import { getAcademicYearSchoolYearLabel, getActiveAcademicYear } from '../../utils/academicYears';
import { 
  StudyGroup,
...
async function getClassroomOptions(trx: any, yearId?: string) {
  const activeYear = yearId ? await trx('academic_years').where({ id: yearId }).first() : await getActiveAcademicYear(trx);
  const schoolYearLabel = getAcademicYearSchoolYearLabel(activeYear);
  const query = trx('homerooms as h')
  ...
  if (schoolYearLabel) {
    query.andWhere('h.school_year', schoolYearLabel);
  }
...
function normalizeWeeklySchedule(weeklySchedule: unknown): WeeklyScheduleEntry[] {
  ...
  const normalizedEntries = weeklySchedule
    ...
    .filter((entry) => isValidTimeRange(entry.start_time, entry.end_time))
    .sort((left, right) => left.day_of_week - right.day_of_week || left.start_time.localeCompare(right.start_time));

  const uniqueEntries = new Map<string, WeeklyScheduleEntry>();

  for (const entry of normalizedEntries) {
    uniqueEntries.set(`${entry.day_of_week}:${entry.start_time}:${entry.end_time}`, entry);
  }
...
function getRoomBlockers(
  ...
  existingAssignments: ExistingAssignmentRow[],
  plannedAssignments: PlannedRoomAssignment[]
): { existing: ExistingAssignmentRow[]; planned: PlannedRoomAssignment[] } {
  const existing = existingAssignments.filter((assignment) =>
    assignment.room_id === roomId &&
  ...
  const planned = plannedAssignments.filter((assignment) =>
    assignment.room.id === roomId &&
    assignment.unit.date === date &&
    timeRangesOverlap(assignment.unit.start_time, assignment.unit.end_time, startTime, endTime)
  );
...
function isEnglishStudyGroupBlocker(
  blocker: ExistingAssignmentRow | PlannedRoomAssignment,
  studyGroupTypeById: Map<string, SchedulableStudyGroup['group_type']>
): boolean {
  if ('unit' in blocker) {
    return blocker.unit.group.group_type === 'english';
  }

  if (blocker.assignable_type !== 'study_group' || !blocker.assignable_id) {
    return false;
  }

  return studyGroupTypeById.get(String(blocker.assignable_id)) === 'english';
}

function shouldWarnAboutEnglishPairsRoomUnavailability(
  englishPairsRooms: RoomAvailabilityRow[],
  date: string,
  startTime: string,
  endTime: string,
  existingAssignments: ExistingAssignmentRow[],
  ...
  if (englishPairsRooms.length === 0) {
    return true;
  }

  return englishPairsRooms.some((room) => {
    const blockers = getRoomBlockers(
      room.id,
      date,
      startTime,
      endTime,
    ...
    const allBlockers = [...blockers.existing, ...blockers.planned];

    if (allBlockers.length === 0) {
      return true;
    }

    return allBlockers.some((blocker) => !isEnglishStudyGroupBlocker(blocker, studyGroupTypeById));
  });
}
...
router.post('/schedule', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  ...
  weekly_schedule 
    ...
    const academicYearStartDate = extractAcademicYearDate(activeYear, ['start_date', 'starts_at', 'academic_start_date', 'begin_date']);
    const todayDateOnly = getTodayDateOnly();
    const requestedStartDate = start_date || academicYearStartDate;
    const resolvedStartDate = requestedStartDate && requestedStartDate > todayDateOnly
      ? requestedStartDate
      : todayDateOnly;
    ...
    let groupsForScheduling = enrichedGroups;

if (use_custom_schedule && weekly_schedule) {
  const customSchedule = normalizeWeeklySchedule(weekly_schedule);

  groupsForScheduling = enrichedGroups.map(group => ({
    ...group,
    weekly_schedule: customSchedule
  }));
}
    ...
    if (result.assignments.length > 0) {
      ...
      await db.transaction(async (trx) => {
        const rooms = await trx('rooms')
          ...
          .where({ is_active: true })
          .orderBy('room_number', 'asc') as RoomAvailabilityRow[];
        const englishPairsRooms = rooms.filter(
          (room) => String(room.room_type || '').toUpperCase() === 'ENGLISH_PAIRS'
        );
        ...
        const studyGroupTypeById = new Map<string, SchedulableStudyGroup['group_type']>(
          enrichedGroups.map((group) => [String(group.id), group.group_type])
        );

        const missingAssignedStudyGroupIds = Array.from(
          new Set(
            existingAssignments
              ...
              .filter((groupId) => !studyGroupTypeById.has(groupId))
          )
        );

        if (missingAssignedStudyGroupIds.length > 0) {
          const existingStudyGroups = await trx('groups')
            .select('id', 'subject')
            .whereIn('id', missingAssignedStudyGroupIds);

          existingStudyGroups.forEach((group: { id: string; subject?: string | null }) => {
            studyGroupTypeById.set(String(group.id), mapSubjectToFrontendType(group.subject ?? undefined));
          });
        }
        ...
        for (const assignment of result.assignments) {
          ...
          if (group.group_type === 'english') {
            ...
            if (englishPairsRoom) {
              ...
              plannedAssignments.push({ unit, room: englishPairsRoom });
              existingAssignments.push({
                id: extraAssignmentId,
                room_id: englishPairsRoom.id,
                assignable_id: assignmentToPersist.assignable_id,
              ...
              if (shouldWarnAboutEnglishPairsRoomUnavailability(
                englishPairsRooms,
                unit.date,
                unit.start_time,
                unit.end_time,
                ...
                persistenceWarnings.push(
                  `לא נמצא חדר פנוי מסוג אנגלית זוגות עבור ${group.name} בתאריך ${unit.date} בין ${unit.start_time} ל-${unit.end_time}.`
                );
              }
            }
          }
...
router.post('/export-assignments-report', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const requestedGroupIds = Array.isArray(req.body?.group_ids)
      ? req.body.group_ids.map((id: unknown) => String(id)).filter(Boolean)
      : [];
    ...
    if (activeYear?.id) {
      groupsQuery.where('grp.year_id', activeYear.id);
    }

    if (requestedGroupIds.length > 0) {
      groupsQuery.whereIn('grp.id', requestedGroupIds);
    }

    const rawGroups = await groupsQuery;
    const enrichedGroups = await enrichGroups(rawGroups, db);
    const exportGroupIds = enrichedGroups.map((group: any) => String(group.id));
    ...
    if (exportGroupIds.length === 0) {
      return res.json({
        ...
        data: format === 'csv'
          ? {
              report_content: '\uFEFFשם הקבצה,סוג,שכבה,חדר,סוג חדר,תאריך,שעת התחלה,שעת סיום,סטטוס\n',
              file_name: `study_groups_assignments_${toDateString(new Date())}.csv`
            }
          : {
              assignments: []
            }
    ...
    const groupById = new Map(
      enrichedGroups.map((group: any) => [String(group.id), group])
    );

    const assignments = await db('assignments as a')
      .joinRaw('inner join rooms as r on a.room_id::text = r.id::text')
      .select(
        'a.id',
        'a.assignable_id',
        'a.assignable_type',
      ...
      .where('a.assignable_type', 'study_group')
    ...
    const exportGroupIdsSet = new Set(exportGroupIds.map((id) => String(id)));

    const reportRows = assignments
      .filter((assignment: any) => exportGroupIdsSet.has(String(assignment.assignable_id)))
      .map((assignment: any) => {
      const group = groupById.get(String(assignment.assignable_id));
      return {
        group_id: String(assignment.assignable_id),
        group_name: group?.name || '',
        group_type: getGroupTypeLabel(group?.group_type),
      ...
      left.group_name.localeCompare(right.group_name, 'he') ||
      left.date.localeCompare(right.date) ||
      left.start_time.localeCompare(right.start_time) ||
      left.room_number.localeCompare(right.room_number, 'he')
    );
    ...
    if (format === 'json') {
      return res.json({
        success: true,
        data: {
          has_rows: reportRows.length > 0,
          assignments: reportRows
        }
      });
    }
    ...
    const headers = ['שם הקבצה', 'סוג', 'שכבה', 'חדר', 'סוג חדר', 'תאריך', 'שעת התחלה', 'שעת סיום', 'סטטוס'];
    const csvRows = [
      headers.join(','),
      ...reportRows.map((row) => ([
        row.group_name,
